...
summary: "理解你的 AI 编程助手如何'看见'你的项目——CLAUDE.md、Rules、Agents、Skills 的加载时机与优先级全解。"
date: 2026-02-14
...
image:
  caption: 'Claude Code Preloading Mechanism'

cover:
  image: "https://images.unsplash.com/photo-1555949963-aa79dcee981c?q=80&w=2560"
  position:
    x: 50
    y: 40
  overlay:
...
> [!IMPORTANT]
> 只有 **CLAUDE.md（根目录）** 和 **Rules** 是启动时自动加载的，其他都是按需触发。
...
> [!WARNING]
> 只有**项目根目录**（git root）下的 `.claude/rules/` 会被识别。子目录中的 `.claude/rules/` **永远不会被加载**，即使加了 `alwaysApply: true` 的 frontmatter 也不行。
